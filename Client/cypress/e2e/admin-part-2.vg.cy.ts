...
it('should be possible to add products', () => {
  ...
  cy.location('pathname').should('match', new RegExp('^/admin/(product|produkt)/(new|ny)'));
...
it('should be possible to edit a product', () => {
  ...
  cy.viewport('samsung-s10');
  cy.get('[data-cy="product"]').first().find('[data-cy="admin-edit-product"]').click();
  cy.ensureNoHorizontalScroll();

  cy.location('pathname').should('match', new RegExp(`^/admin/(product|produkt)/.*`));
  cy.reload(); // to ensure routing is used!
  ...
  cy.get('[data-cy="admin-add-product"]').click();
  cy.addOrEditProduct({ title: 'Cypress', defaults: true });
  cy.get('[data-cy="product"]').last().find('[data-cy="admin-edit-product"]').click();
  cy.ensureNoHorizontalScroll();
...
it('should not be possible to edit products with incorrect values', () => {
  cy.viewport('samsung-s10');
  cy.get('[data-cy="product"]').get('[data-cy="admin-edit-product"]').first().click();

  // Title
  cy.addOrEditProduct({ title: '', description: '', price: '', image: '', clear: true });
  cy.shouldDisplayError('product-title-error', 'product-form');
