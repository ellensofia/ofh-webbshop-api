...
export async function loginUser(agent: SuperAgentTest | SuperTest<any>, email = "test@email.com", password = "123123") {
  return await agent.post("/api/users/login").set("content-type", "application/json").send({ email, password });
}

export function expectDocumentListsToBeTheSame<T extends { _id: any }>(list: T[], otherlist: T[]) {
